...
def parse_args():
    ...
    p.add_argument("--task", type=str, required=True, choices=["TOFU", "ScienceQA", "RETURN"])
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        if f1 > best_f1:
            ...
            torch.save({
                ...
                "task": task,
                "stage": stage,
            }, os.path.join(SAVE_DIR, "mlp_best.pt"))
            print("  ↳ Saved best model (by F1) to: ", os.path.join(SAVE_DIR, "mlp_best.pt"))
    torch.cuda.synchronize()
    end_time = time.time()
