...
# ─────────────────────────────────────────────────────────────────────────────
ID_MAPPING_PATH: Path | None = None  # set via CLI if provided
...
def build_commonsense_prompt(question: str, choices: List[Tuple[str, str]]) -> str:
...
def eval_commonsenseqa(model, tok, split: str = "validation", batch_size: int = 4):
    ...
    for batch in tqdm.tqdm(dl, desc=f"Evaluating {split}"):
        ...
        for ex in batch:
            ...
            prompts.append(build_commonsense_prompt(ex["question"], choices))
