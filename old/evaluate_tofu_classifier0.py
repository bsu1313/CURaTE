...
def cross_predict(texts, tokenizer, model, threshold=3):
    ...
    for text in texts:
        # print("text 0 : ", text[0])
        # print("text 1 : ", text[1])
        inputs = tokenizer(text[0], text[1], return_tensors='pt', padding=True, truncation=True).to(device)
...
def eval_subset(model, tok, model_name, name, ds, forget_data, roberta_model, roberta_tok, ID_MAP, batch_size=4):
    ...
    for batch in tqdm.tqdm(dl, desc=f"Eval {name}"):
        ...
        for item in batch:
            ...
            cross_prompts = [[f_info, question] for f_info in ref_q]
            predictions = cross_predict(cross_prompts, roberta_tok, roberta_model)
            # print("cross_prompts: ", cross_prompts)
            # print("predictions: ", predictions)
...
def main():
    ...
    cross_model = AutoModelForSequenceClassification.from_pretrained(model_name)
    cross_tok = AutoTokenizer.from_pretrained(model_name)
